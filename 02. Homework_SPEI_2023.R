...
# ESERCIZIO 2 ---------
# Introduzione -------------------------------------------------------------
df2 = read.csv("/Users/thomasdemassari/Documents/Università/Statistica, probabilità e inferenza/Homework/2004 statewide crime.txt", header = TRUE)
...
lmy = lm(y ~ x2)
lmx1 = lm(x1 ~ x2)
PR.x2 = lm(lmy$res ~ lmx1$res); summary(PR.x2) #regressione parziale controllando per tasso di scolarizzazione
par(mfrow = c(1,2))
colors = rep("black", length(y)); colors[51] = "red"
plot(lmx1$res, lmy$res, xlab = "Redisui del modello 'tasso di povertà' regredito su 'percentuale di diplomati'", 
...
par(mfrow = c(1,1))
#Commento: il grafico a sinistra rappresenta la relazione tra il tasso di omicidi e il tasso di povertà esludendo gli effetti della percentuale di diplomati, mentre 
...
summary(lm2)
alpha = as.numeric(lm2$coefficients[1]); beta1 = as.numeric(lm2$coefficients[2]); beta2 = as.numeric(lm2$coefficients[3])
...
#ogni di un punto percentuale del tasso di povertà in più ci sono 1.6 omicidi in più). 
...
alphaNoDC = as.numeric(lm2NoDC$coefficients[1]); beta1NoDC = as.numeric(lm2NoDC$coefficients[2]); beta2NoDC = as.numeric(lm2NoDC$coefficients[3])
...
plot(lmx1NoDC$res, lmyNoDC$res, xlab = "Redisui del modello 'tasso di povertà' regredito su 'percentuale di diplomati'", 
     ylab = "Residui del modello 'tasso di omicidi' regredito su 'percentuale di diplomati'",
...
plot(lmx2NoDC$res, lmyyNoDC$res, xlab = "Redisui del modello 'percentuale di diplomati' regredito su 'tasso di povertà'", 
     ylab = "Residui del modello 'tasso di omicidi' regredito su 'tasso di povertà'",
...
#come sembrava inzialmente (includendo DC), mentre la relazione tra tasso di omicidi e percentuali di diplomati all'high school è invertita di segno
#Questo è coerente con quanto dimonstrato precedentemente, ovvero che all'aumentare del tasso di scolarizzazione cala il tasso di omicidi
...
df = read.csv("/Users/thomasdemassari/Documents/Università/Statistica, probabilità e inferenza/Homework/lavoro.csv", header = TRUE)
...
df4 = read.csv("/Users/thomasdemassari/Documents/Università/Statistica, probabilità e inferenza/Homework/dati_controlli.csv",header=TRUE)
...
m = read.csv("/Users/thomasdemassari/Documents/Università/Statistica, probabilità e inferenza/Homework/COVID - maschi.csv")
f = read.csv("/Users/thomasdemassari/Documents/Università/Statistica, probabilità e inferenza/Homework/COVID - femmine.csv")
